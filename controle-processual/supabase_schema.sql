...
-- Tabela de Tempo de APN
create table if not exists tempo_apn (
  id bigint primary key,
  processo text not null,
  data_distribuicao date,
  denuncia date,
  ...
  tramitacao_congelada integer,
  updated_at timestamptz default now()
);

-- Migração: garante as colunas novas em bases já existentes
alter table tempo_apn add column if not exists status text default 'Ativo';
alter table tempo_apn add column if not exists tramitacao_congelada integer;

-- Habilita Row Level Security
alter table processos_idosos enable row level security;
alter table cartas_precatorias enable row level security;
alter table tempo_apn enable row level security;
...
create policy "public access" on tempo_apn for all using (true) with check (true);
...
alter publication supabase_realtime add table tempo_apn;
